...
function Face({ pips, size }: { pips: Pip; size: number }) {
  return (
    <svg viewBox="0 0 100 100" width={size} height={size} className="block">
      <rect x="2" y="2" width="96" height="96" rx="8" fill="var(--hq-cream)" stroke="var(--hq-ink)" strokeWidth="4" />
      {PIP_POSITIONS[pips].map(([x, y], i) => (
        <circle key={i} cx={x * 100} cy={y * 100} r="9" fill="var(--hq-ink)" />
      ))}
    </svg>
  );
}
...
export function DominoTile({
  a, b, size = 42, orientation = "h", playable, selected, hidden, onClick, className,
}: {
  ...
  return (
    <button
      ...
      style={{ width: w, height: h }}
      className={cn(
        "ink-border rounded-lg bg-hq-cream flex overflow-hidden relative",
